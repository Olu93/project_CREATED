...
def generate_latex_table(all_results, index, suffix="", caption=""):
    ...
    df.iloc[:, 4] = df.iloc[:, 4].str.replace("_", "-")
...
# %%
rapper_name = 'ES_EGW_SBI_ES_OPC_SBM_HR_IM'
all_results = zip_fa_with_cf(reader, dict_with_cases, factuals, rapper_name)
df, df_styled, df_latex = generate_latex_table(all_results.groupby("fa_id").tail(1), 0, "evo", caption)
# save_table(df_latex, "example_cf1")
display(df_latex)
...
# %%
rapper_name = 'CBG_CBGW_IM'
caption = "Thid counterfactuals was generated by the case-based model. The counterfactual seems far more viable than the one generated by the evolutionary algorithm."
all_results = zip_fa_with_cf(reader, dict_with_cases, factuals, rapper_name)
...
for rapper_name in dict_with_cases.keys():
    ...
    model_stats_dice4el.append(create_stat(rapper_name, "Resource", "Plausibility", np.mean(cf_activities.isin(all_possible_resources))))
...
stats_df_for_paper = stats_df.pivot(values=["Value"], index=["Model", "Dimension"], columns=["Property"]).drop(["ES_EGW_SBI_ES_OPC_SBM_HR_IM", "ES_EGW_SBI_ES_OPC_SBM_RR_IM"], axis=0).droplevel(0, axis=1)#.droplevel("Property")
stats_df_for_paper
# %%
