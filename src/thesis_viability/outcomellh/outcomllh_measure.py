...
class ImprovementMeasure(MeasureMixin):
    ...
    def _pick_probs(self, fa_cases: Cases, cf_cases: Cases) -> Tuple[NDArray, NDArray]:
    ...
    def _compute_diff(self, original_probs: NDArray, new_cf_probs: NDArray) -> NDArray:
...
#     @abstractmethod
#     def compute_diff(self, original_probs: NDArray, counterfactual_probs: NDArray) -> NDArray:
#         pass
...
#     def compute_diff(self, original_probs: NDArray, new_cf_probs: NDArray) -> NDArray:
#         len_fa, len_cf = original_probs.shape[0], new_cf_probs.shape[0]
#         fa_counter_probs = (1 - original_probs)  # counter probaility of factual as we want to invert those
#         new_cf_probs = new_cf_probs.T
#         fa_counter_outcomes = fa_counter_probs > .5  # The direction to go
...
# class Picker(ABC):
#     @abstractmethod
#     def pick_probs(self, original_probs: NDArray, new_cf_probs: NDArray) -> Tuple[NDArray, NDArray]:
#         pass
