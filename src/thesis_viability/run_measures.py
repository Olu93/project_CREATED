...
if __name__ == "__main__":
    ...
    if DEBUG_SPARCITY:
        print("Run Sparcity")
        sparcity_computer: SparcityMeasure = SparcityMeasure().set_vocab_len(vocab_len).set_max_len(max_len)
        sparcity_values = sparcity_computer.compute_valuation(fa_cases, cf_cases).normalize()
        print(sparcity_values)

    if DEBUG_SIMILARITY:
        print("Run Similarity")
        similarity_computer: SimilarityMeasure = SimilarityMeasure().set_vocab_len(vocab_len).set_max_len(max_len)
        similarity_values = similarity_computer.compute_valuation(fa_cases, cf_cases).normalize()
        print(similarity_values)

    if DEBUG_DLLH:
        print("Run Data Likelihood")
        dllh_computer: DatalikelihoodMeasure = DatalikelihoodMeasure().set_training(tr_cases).set_vocab_len(vocab_len).set_max_len(max_len)
    ...
    if DEBUG_OLLH:
        ...
        improvement_computer: OutcomelikelihoodMeasure = OutcomelikelihoodMeasure().set_predictor(prediction_model).set_vocab_len(vocab_len).set_max_len(max_len)
