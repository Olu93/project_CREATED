...
from thesis_commons.config import DEBUG_USE_MOCK, MAX_ITER_STAGE_SPECIAL, READER
...
def create_combinations(erate: float, mrate: MutationRate, evaluator: ViabilityMeasure):
    ...
    crossers = [
        evolutionary_operations.UniformCrosser().set_crossover_rate(0.25),
        evolutionary_operations.UniformCrosser().set_crossover_rate(0.50),
        evolutionary_operations.UniformCrosser().set_crossover_rate(0.75),
    ]
...
if __name__ == "__main__":
    ...
    all_iterations = [5] if DEBUG_QUICK_MODE else MAX_ITER_STAGE_SPECIAL*2
    # all_iterations = [5] if DEBUG_QUICK_MODE else [2, 5]
    top_k = 10 if DEBUG_QUICK_MODE else 50
    # sample_size = max(top_k, 100) if DEBUG_QUICK_MODE else max(top_k, 1000)
    sample_size = 200 
    num_survivors = 1000
    experiment_name = "evolutionary_uniforms"
