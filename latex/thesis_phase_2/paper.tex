...
% \makeindex
\addbibresource{./references/auto-updated.bib}
...
% TODO: Apply the title case to all chapters, sections and subsections.
...
% TODO: Add a colour matrix here to show the level of correlation
...
% Before testing any model, we must establish two crucial components of the viability measure. First, we require a prediction model, which we want to explain using counterfactuals. This is relevant for determining the improvement of counterfactual yields compared to the factual. Second, we need to know to what extent any given counterfactual is feasible given the dataset. Therefore, we dedicate the first set of experiments to establishing these components.
...
% With these results, we cannot claim that the model consistently outperforms the other approaches. Therefore, this section explores the results of different dataset sizes and types.
