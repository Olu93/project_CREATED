...
\label{sec:exp2}
Given the chosen configuration in \autoref{sec:exp1}, we will now explore optimal parameter settings. Therefore, we run the model \attention{50} times with different mutation-rates. For the chosen, we uniformly sample the mutation rates for \emph{delete}, \emph{insert} and \emph{change} between 0 and 1 for each run. The remaining procedure follows the process described in \autoref{sec:exp1}.
