...
We see that \optional{CBI-ES-UC3-SBM-RR} returns the best results in the shortest time span. The model converges after roughly 50 iterative cycles. \attention{CBI-RWS-OPC-SBM-BBR} appears to have not reached convergence. 
...
Case-based initiated models appear to slightly gain more events. Although,  \optional{CBI-RWS-OPC-SBN-BBR} appears that reaches its saturation point significantly later (\optional{100}th cycle).
...
Another surprise is the steep ascention of only model that uses tournament selection (\optional{RI-TS-TPC-SBM-RR}) towards the end of the generation process. The  model even overtakes the model that leads the model-configurations in terms of \attention{viability}. 
