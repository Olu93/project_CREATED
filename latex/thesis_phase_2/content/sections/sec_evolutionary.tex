...
Our goal is to generate candidates by modifying the factual sequence and evaluating the sequence based on our viability measure. This measure acts as a fitness function. The cadidates that are deemed fit enough are subsequently selected reproduce offspring. The offspring is subject to mutations.Then we evaluate the new population repeat the procedure until a termination condition is reached. It differs from \glossary{DL}, because it does not require us to use differantiable functions. Hence, we can directly optimise the viability measure established in \autoref{sec:viability}.
