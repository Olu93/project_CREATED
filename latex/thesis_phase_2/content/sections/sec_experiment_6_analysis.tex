\documentclass[./../../paper.tex]{subfiles}
\graphicspath{{\subfix{./../../figures/}}}

\begin{document}

Based on these results, we can see that our model does seem to optimize properly under our viability function. If we compare our results with a different set of results, we see they often compare or even outperform the alternative solution. 
Unsurprisingly, the \ModelCBG achieves the highest plausibility as all the counterfactuals were drawn from the data. 

% In terms of the very similar resource proximity and sparsity scores, we assume that the model can identify the correct resource given the activity. For instance, if within a loan approval process, only one person or machine executes the activity of checking the identity of an applicant, then \ModelEVOFSR seems capable of learning this relationship.  


\end{document}