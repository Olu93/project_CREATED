...
% We see, also see their event attributes. These could be related to the applicants credit score, their yearly income or other case related information. Hence, one could say that the first events in both cases are identical. If our sparsity measure only counts the number of changes of the first event, we get a sparsity-cost of 0. However, this is a sequence and the next event does display a change. The first dimension of the second event on the factual side is 0.25, while it is 0.75 on the counterfactual. It is clear that the sparsity is worse on the second event. In fact, according to \autoref{eq:sparcity_measure} we now have a sparsity cost of 1. The third events in both sequences are identical again. Hence, no cost. However, it is difficult to argue that the cost of both sequences is just 1. Mainly because the counterfactual has a fourth event. This difference between the two sequences reveals the improtance of structural differences between sequences. Mainly, if we do not take the structure into account, we run the risk of misrepresenting the distance, we perceive.  
