...
\begin{itemize}
    ...
    \item[Feasibility:] Each counterfactual should be feasible. As an example, a sequence \textit{ABCDE\textbf{1G}} would not be feasible if numericals are not allowed. \optional{Typically we can use data to ensure this property. However, the \emph{open-world assumption} makes this criterion quite hard to fulfill. We have to expect longer sequences that have not been encountered, yet.}
\end{itemize}
...
We refer to both sets of validity criterions as \emph{individual validity} and \emph{population validity}. However, to remains concise, we will use \emph{validity} to refer to the individual criterions only. We will explicitly mention \emph{population validity} if we refer to criterions that concern the poulation.
