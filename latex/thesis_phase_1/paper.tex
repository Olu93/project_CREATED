...
Let $\mathcal{E}$ be the universe of event identifiers and $E \subseteq \mathcal{E}$ a set of events. Let $C$ be a set of case identifiers and $\pi_\sigma : E \mapsto C$ a surjective function that links every element in $E$ to a case in $C$. For a set of events $E \subseteq \mathcal{E}$ we define $\sigma \in E$ being a particular sequence $\sigma = \langle e_1, e_2, \ldots, e_n \rangle$ as a case of length $n$. 
...
We represent each event as a concatenated vector of its attribute values as $e_n^{\sigma} = [f_1^{\sigma}; f_2^{\sigma}; \ldots; f_i^{\sigma}]$, in which $\sigma$ specifies a particular case, $n$ a specific event within the case and $i$ its data attributes. 

\needsfigure{fig:representation}{This figure show the representation of a particular event $e$ and a case $\sigma$.}
